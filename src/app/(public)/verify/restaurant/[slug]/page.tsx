...
export async function generateMetadata({ params }: { params: { slug: string } }) {
  return {
    title: `Verify Restaurant | ${params.slug}`,
    description: `Water quality verification for ${params.slug}`
  };
}

export default async function VerifyRestaurantPage({ params }: { params: { slug: string } }) {
  const result = await getOrganizationBySlug(params.slug);

  if (!result.success || !result.organization || result.organization.type !== "RESTAURANT") {
    notFound();
  }
  ...
  return (
    <div className="container mx-auto py-12 px-4 max-w-3xl">
      ...
      <Card>
        <CardHeader>
          <CardTitle>Restaurant Verification Status</CardTitle>
        </CardHeader>
